...
`,
	...
	Run: func(cmd *cobra.Command, args []string) {
		logger, err := utils.NewLogger(os.Stdout, "root")
		if err != nil {
			logger.Fatal(err.Error())
			panic(err)
		}

		logger.Fatalf("Missing sub-command? Run -h for more info.")
	},
}
...
func initConfig() {
	if cfgFile != "" {
		...
		// Search config in home directory with name ".grocy-alerts" (without extension).
		viper.AddConfigPath(home)
		viper.SetConfigType("yaml")
		viper.SetConfigName(".grocy-alerts")
	}
